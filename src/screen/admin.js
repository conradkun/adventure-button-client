...
class Admin extends Component {
  constructor(props) {
    ...
    **/
    ...
    organisation.on("patched", org => {
      const newOrganisationList = this.state.organisations.filter(o => {
        return o._id !== org._id;
      });
      newOrganisationList.push(org);
      this.setState({
        organisations: newOrganisationList
      });
    });
    organisation.on("created", org =>
      this.setState({
    ...
    this._onRequestForEditUserClose = this._onRequestForEditUserClose.bind(
      this
    );
    this._onRequestForDeleteUser = this._onRequestForDeleteUser.bind(this);
    this._onRequestForDeleteUserClose = this._onRequestForDeleteUserClose.bind(
      this
    );
    ...
    this._onRequestForAddOrganisation = this._onRequestForAddOrganisation.bind(
      this
    );
    this._onRequestForAddOrganisationClose = this._onRequestForAddOrganisationClose.bind(
      this
    );
    this._onRequestForEditOrganisation = this._onRequestForEditOrganisation.bind(
      this
    );
    this._onRequestForEditOrganisationClose = this._onRequestForEditOrganisationClose.bind(
      this
    );
    this._onRequestForDeleteOrganisation = this._onRequestForDeleteOrganisation.bind(
      this
    );
    this._onRequestForDeleteOrganisationClose = this._onRequestForDeleteOrganisationClose.bind(
      this
    );
  ...
  _onAddUser(User) {
    ...
    if (User.password) {
      users
        ...
        .then(this.setState({ addUser: false }));
    } else {
      users
        .create({
          email: User.email,
  ...
  _onAddOrganisation(Organisation) {
    ...
    const organisation = client.service("organisation");
    organisation
      .create({ name: Organisation.name, seats: Organisation.seats })
      .then(this.setState({ addOrganisation: false }));
  }

  _onRequestForEditOrganisation(
    id = this.state.editOrganisationOption.id,
    name = this.state.editOrganisationOption.name,
    seats = this.state.editOrganisationOption.seats,
    minSeats = this.state.editOrganisationOption.minSeats
  ) {
    this.setState({
  ...
  _onEditOrganisation(id, Organisation) {
    ...
    const organisation = client.service("organisation");
    organisation
      .patch(id, { name: Organisation.name, seats: Organisation.seats })
      .then(this.setState({ editOrganisation: false }));
  }

  _onRequestForDeleteOrganisation(
    id = this.deleteOrganisationOption.id,
    name = this.deleteOrganisationOption.name
  ) {
    this.setState({
  ...
  _search(organisation) {
    ...
    return (
      name.toLowerCase().indexOf(this.state.searchString.toLowerCase()) !== -1
    );
  }
  _load() {
    const client = this.props.client;
  ...
  _renderContent() {
    ...
    cards = organisations.filter(this._search).map(org => {
      ...
      return (
        <OrganisationCard
          ...
          seats={org.seats}
          userCount={userCount}
          onAddUser={this._onRequestForAddUser}
          onEditOrganisation={this._onRequestForEditOrganisation}
          onDeleteOrganisation={this._onRequestForDeleteOrganisation}
          onDeleteUser={this._onRequestForDeleteUser}
          onPromoteUser={this._onPromoteUser}
        />
      );
    });
    ...
    if (this.state.addUser) {
      modal = (
        <AddUserModal
          msg={this.props.msg}
          organisation={this.state.addUserOption.organisation}
          onClose={this._onRequestForAddUserClose}
      ...
      modal = (
        <EditUserModal
          id={this.state.editUserOption.id}
      ...
      modal = (
        <DeleteUserModal
          onClose={this._onRequestForDeleteUserClose}
          id={this.state.deleteUserOption.id}
          email={this.state.deleteUserOption.email}
      ...
      modal = (
        <AddOrganisationModal
          msg={this.props.msg}
          onClose={this._onRequestForAddOrganisationClose}
      ...
      modal = (
        <DeleteOrganisationModal
          onClose={this._onRequestForDeleteOrganisationClose}
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Anchor
            icon={<Add />}
            onClick={this._onRequestForAddOrganisation}
          />{" "}
          {mobileButton}
        </Box>
      </Header>
